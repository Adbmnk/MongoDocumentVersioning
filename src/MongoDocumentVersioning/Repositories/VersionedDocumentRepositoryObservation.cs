...
{
    public class VersionedDocumentRepositoryObservation<T> where T : class, IObservationKey
    {
        private readonly MongoDbContext _dbContext;
        public IMongoCollection<VersionedDocumentObservation<T>> Collection { get; private set; }
        private readonly JsonDiffPatch _jdp = new JsonDiffPatch();

        public VersionedDocumentRepositoryObservation(MongoDbContext dbContext)
        {
            _dbContext = dbContext;
            Collection = _dbContext.MongoDbCollection<VersionedDocumentObservation<T>>();
        }
        ...
        public async Task InsertDocumentsAsync(List<T> speciesObservations)
        {
            ...
            {
                List<VersionedDocumentObservation<T>> newVersionedObservations = new List<VersionedDocumentObservation<T>>();
                foreach (var observation in newObservations)
                {
                    newVersionedObservations.Add(new VersionedDocumentObservation<T>(observation));
                }
        ...
        {
            List<ObservationIdTuple> foundObservationsTuples = await FindObservationKeysAsync(speciesObservations);
            HashSet<ObservationIdTuple> foundObservationIdsSet = new HashSet<ObservationIdTuple>(foundObservationsTuples);
            var newObservations = speciesObservations.Where(x =>
                    !foundObservationIdsSet.Contains(new ObservationIdTuple(x.DataProviderId, x.CatalogNumber)))
        ...
        private async Task<List<ObservationIdTuple>> FindObservationKeysAsync(List<T> speciesObservations)
        {
            ...
            var options = new FindOptions<VersionedDocumentObservation<T>, ObservationIdTuple> { Projection = projection };
        ...
        public async Task<VersionedDocumentObservation<T>> GetDocumentAsync(int dataProviderId, string catalogNumber)
        {
        ...
        public async Task<VersionedDocumentObservation<T>> GetDocumentAsync(ObjectId objectId)
        {
